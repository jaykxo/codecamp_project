window.onload = () => {
  ...
  const 감정카드생성 = (기분, 이미지경로, 제목, 날짜, 인덱스) => {
    ...
    카드.innerHTML = `
      <div class="일기사진">
        <img class="기분이미지" src="${이미지경로}" alt="${기분}" />
      </div>
      <div class="일기내용">
        <div class="기분 ${기분클래스}">${기분}해요</div>
        <div class="날짜">${날짜}</div>
      </div>
      <div class="일기제목">${제목}</div>
    `;
  ...
  const 글쓰기기능 = () => {
    ...
    const 제목담는통 = window.document.getElementById("제목입력창").value;
    const 내용담는통 = window.document.getElementById("내용입력창").value;
  
    // 1-2. 오늘의 기분 불러오기
    let 행복담는통 =
      window.document.getElementsByName("기분선택버튼")[0].checked === true;
    let 슬픔담는통 =
      window.document.getElementsByName("기분선택버튼")[1].checked === true;
    let 놀람담는통 =
      window.document.getElementsByName("기분선택버튼")[2].checked === true;
    let 화남담는통 =
      window.document.getElementsByName("기분선택버튼")[3].checked === true;
    let 기타담는통 =
      window.document.getElementsByName("기분선택버튼")[4].checked === true;
    ...
    const 기존의일기도화지 =
      window.document.getElementById("일기보여주는곳").innerHTML;
  
    // 5. 새로운 일기도화지 만들기
    const 새로운일기도화지 = `
      <div class="일기" onclick="글보기기능(${일기번호 + 6})">
          <div class="일기사진">
            ${
              행복담는통 === true
                ? '<img class="기분이미지" src="./assets/images/joy.png" alt="행복" />'
                : ""
            }
            ${
              슬픔담는통 === true
                ? '<img class="기분이미지" src="./assets/images/sadness.png" alt="슬픔" />'
                : ""
            }
            ${
              놀람담는통 === true
                ? '<img class="기분이미지" src="./assets/images/surprised.png" alt="놀람" />'
                : ""
            }
            ${
              화남담는통 === true
                ? '<img class="기분이미지" src="./assets/images/anger.png" alt="화남" />'
                : ""
            }
            ${
              기타담는통 === true
                ? '<img class="기분이미지" src="./assets/images/idontknownothing.png" alt="기타" />'
                : ""
            }
          </div>
          <div class="일기내용">
            ${
              행복담는통 === true
                ? `<div class="기분 행복">행복해요</div>`
                : ""
            }
            ${
              슬픔담는통 === true
                ? `<div class="기분 슬픔">슬퍼요</div>`
                : ""
            }
            ${
              놀람담는통 === true
                ? `<div class="분 놀람">놀랐어요</div>`
                : ""
            }
            ${
              화남담는통 === true
                ? `<div class="기분 화남">화나요</div>`
                : ""
            }
            ${
              기타담는통 === true
                ? `<div class="기분 기타">기타</div>`
                : ""
            }
            <div class="날짜">${일기담는통.작성일}</div>
          </div>
          <div class="일기제목"> ${일기담는통.제목}</div>
      </div>
    `;
  
    // 6. 일기보여주는곳에 기존의 일기도화지와 새로운 일기도화지 함께 보여주기
    window.document.getElementById("일기보여주는곳").innerHTML =
      기존의일기도화지 + 새로운일기도화지;
